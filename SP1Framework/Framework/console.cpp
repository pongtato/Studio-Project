...
void writeToBuffer(int c,int d, char ch, WORD attribute)
{
    screenDataBuffer[c + ConsoleSize.X * d].Char.AsciiChar = ch;
    screenDataBuffer[c + ConsoleSize.X * d].Attributes = attribute;
}
void writeToBuffer(int c,int d, std::string& s, WORD attribute)
{
	COORD a = {c,d};
    writeToBuffer(a, s.c_str(), attribute);
}
